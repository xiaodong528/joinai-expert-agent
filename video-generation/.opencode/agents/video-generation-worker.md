...
```bash
python .opencode/skills/video-s1-storyboard/scripts/run_stage.py \
...
```bash
PYTHONPATH=.opencode/skills/video-s4-image-to-video/scripts \
python .opencode/skills/video-s4-image-to-video/scripts/stage4_seedance.py \
  --project-id <project_id> \
  --scene <scene>
```
...
```bash
PYTHONPATH=.opencode/skills/video-s5-tts/scripts \
...
```bash
PYTHONPATH=.opencode/skills/video-s6-bgm/scripts \
...
```bash
PYTHONPATH=.opencode/skills/video-s7-concat/scripts \
python .opencode/skills/video-s7-concat/scripts/stage7_concat.py \
  --project-id <project_id>
```
...
```bash
PYTHONPATH=.opencode/skills/video-s8-lipsync/scripts \
python .opencode/skills/video-s8-lipsync/scripts/stage8_lipsync.py \
  --project-id <project_id>
```
...
```bash
PYTHONPATH=.opencode/skills/video-s9-subtitle/scripts \
python .opencode/skills/video-s9-subtitle/scripts/stage9_subtitle.py \
  --project-id <project_id>
```
...
```bash
PYTHONPATH=.opencode/skills/video-s10-qa-review/scripts \
python .opencode/skills/video-s10-qa-review/scripts/stage10_qa.py \
  --project-id <project_id>
```
...
- 所有脚本默认接收 `--project-id`
- 需要脚本相对导入时，优先设置 `PYTHONPATH=.opencode/skills/<skill>/scripts`
