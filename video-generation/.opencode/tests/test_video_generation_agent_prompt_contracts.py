...
class VideoGenerationAgentPromptContracts(unittest.TestCase):
    def test_orchestrator_requires_rig_url_and_gt_polecat_parallelism(self):
        content = read_text(".opencode/agents/video-generation-orchestrator.md")
        ...
        self.assertIn("并行始终表示 GT 中一个或多个 `Polecat` bead / session", content)
        self.assertIn("并行 owner 始终是 GT `Polecat`", content)

    def test_worker_rejects_invalid_url_and_generic_subagents(self):
        content = read_text(".opencode/agents/video-generation-worker.md")
        ...
        self.assertIn("只接受 GT `Polecat` bead / session 派发", content)
        self.assertIn("不接受通用子智能体并行替代", content)

    def test_reviewer_blocks_invalid_rig_url_and_non_polecat_batches(self):
        content = read_text(".opencode/agents/video-generation-reviewer.md")
