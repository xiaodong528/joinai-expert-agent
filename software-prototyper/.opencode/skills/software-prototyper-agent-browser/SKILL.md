---
name: software-prototyper-agent-browser
description: Browser automation CLI for software-prototyper roles. Use when Polecat or Refinery needs to interact with web apps, validate real browser flows, run smoke checks, capture screenshots, or automate browser-based testing in the JAS software-prototyper pipeline.
allowed-tools: Bash(npx agent-browser:*), Bash(agent-browser:*)
---

# Browser Automation with software-prototyper-agent-browser

在 `software-prototyper` 主链里，开发阶段与验收阶段的浏览器自动化测试都统一走这个 skill。

- `Polecat`：用于开发侧 smoke、页面交互验证、真实 Web 流程 smoke、截图取证、浏览器自动化测试
- `Refinery`：用于验收侧独立 walkthrough、独立浏览器验收、关键页面 walkthrough、真实用户流复核
- `Mayor`：不直接执行浏览器自动化，只负责在首次合法节点把浏览器自动化测试节点交接给 `Polecat` 或 `Refinery`
- 失败时必须回流并继续修复，不能跳过
