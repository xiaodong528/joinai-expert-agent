...
**用途（Purpose）:** construction-audit 主链三角色共享的 GT 状态上报技能。在 Gas Town 会话中统一处理角色身份注入、Bead 状态更新和 Worker 完成信号。Gas Town 默认 witness 巡逻若启用，仍由平台默认机制负责。所有状态都以当前 rig 为唯一真值来源。
...
- construction-audit 不再注册 custom witness agent
...
- [ ] 文案明确 Gas Town 默认 witness 不属于 construction-audit 自定义角色注册
