...
| `sheet_scope` | string | 否 | `visible` 或 `all`；默认 `visible`，回归链路可显式指定 `all` |
...
- `spreadsheet.sheet_scope` 记录本轮目标 sheet 选择策略。
- `spreadsheet.sheets` 表示当前运行的目标 sheet 集合，不等于全部物理 sheet。
...
4. 根据 `sheet_scope` 计算 `spreadsheet.sheets`：`visible` 取 `visible_sheets`，`all` 取 `all_sheets`。
...
- [ ] `spreadsheet.sheet_scope` 为 `visible` 或 `all`
- [ ] `spreadsheet.sheets` 非空
- [ ] `sheet_scope=visible` 时，`spreadsheet.sheets` 默认等于 `visible_sheets`；只有无可见 sheet 时才回退到 `all_sheets`
- [ ] `sheet_scope=all` 时，`spreadsheet.sheets == all_sheets`
